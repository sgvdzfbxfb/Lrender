...
{
    Bounds3 ret;
    ret.pMin = Vector3D(std::min(b1.pMin.x, b2.pMin.x), std::min(b1.pMin.y, b2.pMin.y), std::min(b1.pMin.z, b2.pMin.z));
    ret.pMax = Vector3D(std::max(b1.pMax.x, b2.pMax.x), std::max(b1.pMax.y, b2.pMax.y), std::max(b1.pMax.z, b2.pMax.z));
    return ret;
}
...
BVHBuildNode* BVHAccel::recursiveBuild(std::vector<BVHItem*> objects)
{
    ...
    if (objects.size() == 1) {
        ...
        auto leftshapes = std::vector<BVHItem*>(beginning, middling);
        auto rightshapes = std::vector<BVHItem*>(middling, ending);
